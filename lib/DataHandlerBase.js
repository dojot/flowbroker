...
=======
    /**
     * File name to locale with extension
      * @return {string} A string with  the File name to locale with extension
     */
    getLocaleFilename() {
        return "default.json";
    }
...
=======
        let filepath = this.getLocalesPath() + '/' +  locale  + '/'+  this.getLocaleFilename();
        if (fs.existsSync(filepath)) {
            return Promise.resolve(require(filepath));
        } else {
            filepath = path.join(this.getLocalesPath(), defaultLocale , this.getLocaleFilename());
            if (fs.existsSync(filepath)) {
                return Promise.resolve(require(filepath));
            } else {
