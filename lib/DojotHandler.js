...
module.exports = class DojotHandler {
  ...
  init() {
    if (this.isInitialized) {
      logger.warn('CommandHandler is already initialized', {filename: "flownode/dojotHandler"});
      return;
    }
    ...
    this.sock.on("message", (identity, packet) => {
      ...
      try {
        ...
        logger.warn(`Invalid JSON format. Discarding request: ${packet.toString()}. Error: ${error}`,
          {filename: "flownode/dojotHandler"});
        return;
      }
      try {
        ...
        logger.warn(`Exception: ${error}`, {filename: "flownode/dojotHandler"});
        return;
      }
    }); // on message
    ...
    this.sock.bind('tcp://*:' + this.port, (err) => {
      if (err) {
        logger.error(`Failed on bind the zmq port ${this.port}. Error: ${err}`,
          {filename: "flownode/dojotHandler"});
        process.exit(1);
      } else {
        logger.info(`zmq listening on ${this.port}`,
          {filename: "flownode/dojotHandler"});
      }
    });
  ...
  _handleRequest(request) {
    ...
    try {
      ...
      logger.debug(`Got message. invoking handler ... ${util.inspect(request, false, null)}`,
        {filename: "flownode/dojotHandler"});
