...
class DataHandler extends dojot.DataHandlerBase {
  ...
  handleMessage(config, message) {
    ...
    // Pre-process URL.
    var url = nodeUrl || httpRequest.url || message.url;
    ...
    if (!/^(http|https):\/\//.test(url)) {
      ...
      logger.error(
        "Node has an invalid transport protocol (no http nor https).",
        {
          filename: "http",
        }
      );
    ...
    try {
      ...
      var payload = null;
      if (
        typeof httpRequest.payload !== "undefined" &&
        (method === "POST" || method === "PUT" || method === "PATCH")
      ) {
        if (
          typeof httpRequest.payload === "string" ||
          Buffer.isBuffer(httpRequest.payload)
        ) {
      ...
      if (
        opts.headers.hasOwnProperty("content-length") &&
        clSet !== "content-length"
      ) {
      ...
      return new Promise((resolve, reject) => {
        ...
        var req = (/^https/.test(urltotest) ? https : http).request(
          opts,
          (res) => {
            ...
            res.on("data", (chunk) => {
              if (!Buffer.isBuffer(chunk)) {
                ...
                return reject(
                  new Error("HTTP Request data chunk not a Buffer")
                );
              }
                ...
                if (ret !== "bin") {
                  ...
                  if (ret === "obj") {
                    try {
                      ...
                      logger.warn(
                        "Could not parse JSON. Forwarding as plain string."
                      );
                    }
                  }
                }
